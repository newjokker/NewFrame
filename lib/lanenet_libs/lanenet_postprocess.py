...
class _LaneNetCluster(object):
    ...
    def _embedding_feats_dbscan_cluster(self, embedding_image_feats):
        ...
        :return:
        """
        db = DBSCAN(eps=self._cfg.DBSCAN_EPS, min_samples=self._cfg.DBSCAN_MIN_SAMPLES)
        try:
...
class LaneNetPostProcessor(object):
    ...
    """
    def __init__(self, cfg,ipm_remap_file_path='./tusimple_ipm_remap.yml'):
        """
        :param ipm_remap_file_path: ipm generate file path
        """
        ipm_remap_file_path = ops.join(ops.dirname(__file__),'tusimple_ipm_remap.yml')
    ...
    def splitcoords(self,lane_coords):
        start = 0
        coord_list = []
        coord = []
        print('split!!!!!!!!!!!')
        print(lane_coords.shape[0])
        for i in range(lane_coords.shape[0]):
            point = lane_coords[i]
            pointX = point[0]
            #print('start',start)
            #print('pointX',pointX)
            if start <= pointX:
                start = pointX
                coord.append(copy.deepcopy(point))
            else:
                print(coord)
                pointX = 0
                coord_list.append(copy.deepcopy(coord))
                coord = []
        print(coord_list)
        return coord_list
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        #    for i in range(lane_coords.shape[0]):
        #        f.write(str(lane_coords[i]))
        #        f.write('\n')

        lane_coords_list = lane_coords# self.splitcoords(lane_coords)
        ...
        for lane_index, coords in enumerate(lane_coords):
            if data_source == 'tusimple':
                tmp_mask = np.zeros(shape=(2560, 1440), dtype=np.uint8)
                tmp_mask[tuple((np.int_(coords[:, 1] * 1440 / 256), np.int_(coords[:, 0] * 2560 / 512)))] = 255
            else:
