...
class BiseNetV2(cnn_basenet.CNNBaseModel):
    ...
    def __init__(self, phase, cfg):
        ...
        self._class_nums = self._cfg.NUM_CLASSES
        #print('#'*20)
        #print(self._class_nums)
        self._weights_decay = self._cfg.WEIGHT_DECAY
        self._loss_type = self._cfg.LOSS_TYPE
        self._enable_ohem = self._cfg.OHEM.ENABLE
        if self._enable_ohem:
            self._ohem_score_thresh = self._cfg.OHEM.SCORE_THRESH
            self._ohem_min_sample_nums = self._cfg.OHEM_MIN_SAMPLE_NUMS
        self._ge_expand_ratio = self._cfg.BISENETV2_GE_EXPAND_RATIO
        self._semantic_channel_ratio = self._cfg.BISENETV2_SEMANTIC_CHANNEL_LAMBDA
        self._seg_head_ratio = self._cfg.BISENETV2_SEGHEAD_CHANNEL_EXPAND_RATIO
