...
def convert_pascal_to_tfrecord():
    ...
    for count, xml in enumerate(glob.glob(xml_path + '/*.xml')):
        ...
        img_name = xml.split('/')[-1].split('.')[0] + str('.')+xml.split('/')[-1].split('.')[1] + '.jpg'
        if img_name.split('.')[1]=='xml':
           img_name=img_name.split('.')[0]+str('.')+img_name.split('.')[2]
        img_path = image_path + '/' + img_name
        if not os.path.exists(img_path):
            img_name = xml.split('/')[-1].split('.')[0] + str('.')+xml.split('/')[-1].split('.')[1] + '.png'
            if img_name.split('.')[1]=='xml':
               img_name=img_name.split('.')[0]+str('.')+img_name.split('.')[2]
            img_path = image_path + '/' + img_name
