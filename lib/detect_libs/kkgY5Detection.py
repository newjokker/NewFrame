...
import cv2
import numpy as np
from ..detect_utils.utils import mat_inter,compute_iou_min
from ..detect_utils.tryexcept import try_except
from .yolov5Detection import YOLOV5Detection


class KkgDetection(YOLOV5Detection):
    ...
    @try_except()
    def check_label_in(self,tot_label):
        return [obj for obj in tot_label if (obj[0] in self.labeles_checkedIn)]
        ...
        try:
            ...
            for ljcBox in ljcBoxes:
                ...
                if ljcName in kkxBoxes.keys():
                    for kkxBox in kkxBoxes[ljcName]:
                        ...
                        kkxBox[4] += xmap
                        kkxBox[5] += ymap
                        if ('other' in kkxBox[0] or 'dense' in kkxBox[0]):
                            otherList.append((float(kkxBox[2]), float(kkxBox[3]), float(kkxBox[4]), float(kkxBox[5])))
                        ### K KG Lm在其中 ###
                        elif kkxBox[0] in self.labeles_checkedIn:
            ...
            for ljcName, ks in kList.items():
                ...
                for k in ks:
                    self.log.info(k)
                    ## 只过滤K ##
                    if k[0] == 'K' and self.isBoxIn_other(otherList, (float(k[2]), float(k[3]), float(k[4]), float(k[5]))):
    ...
    def solve_coincide_other(self, box1, box2, iouThresh):
        if mat_inter(box1, box2) == True:
            coincide = compute_iou_min(box1, box2)
    ...
    @try_except()
    def draw_voc_boxes(self, im, ljcBox,tot_labels):
            ...
            for i, obj in enumerate(tot_labels):
    ...
    @try_except()
    def draw_all_boxes(self, img_name, ljcBoxes, results,input_dir='./inputImg'):
            ori_name = img_name + '.jpg' if os.path.exists(os.path.join(input_dir, img_name + '.jpg')) else img_name + '.JPG'
            ori_img_path = os.path.join(input_dir, ori_name)
