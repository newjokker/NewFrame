...
from ..detect_libs.fasterDetectionPyTorch import FasterDetectionPytorch


class SgtorchDetection(FasterDetectionPytorch):
    def __init__(self, args, objName, scriptName):
        super(SgtorchDetection, self).__init__(args, objName, scriptName)
    ...
    def postProcess(self, name, ljcBox, detectBoxes, results):
        for dBox in detectBoxes:
            if name in ljcBox['resizedName']:
                ...
                ymax = ymax + ymap
                results.append(
                    {'label': label, 'index': index, 'xmin': xmin, 'ymin': ymin, 'xmax': xmax, 'ymax': ymax, 'prob': p})
        return results
