...
from ..detect_libs.mmsegDetection import mmsegDetection 

class lwSwinTDetection(mmsegDetection):
    def __init__(self, args, objName=None, scriptName=None):
        super(lwSwinTDetection, self).__init__(args,objName, scriptName)
    ...
    def filterPoints(self,raw_points):
        ...
        length = len(result)
        index = np.linspace(0,length,endpoint = False,num=int(length/5),dtype=int)
