...
def dete_jyhQX(model_dict, data):
    ...
    try:
        ...
        # ----------------------------------------------------------------------------------------------------------
        jyhqx_1_dete_res.img_ndarry = data['im']
        for each_dete_obj in jyhqx_1_dete_res:
            each_im = jyhqx_1_dete_res.get_sub_img_by_dete_obj_new(each_dete_obj)
            a = model_jyhqx_2.detect(each_im, data['name'])
            #
            if len(a) < 1:
        ...
        for each_dete_obj in jyhqx_1_dete_res:
            ...
            if each_dete_obj.tag not in ['upring', 'downring'] or each_dete_obj.des in [None, "", " "]:
                each_dete_obj.tag = 'Hnormal'
                continue
            # do dete
            each_im = jyhqx_1_dete_res.get_sub_img_by_dete_obj_new(each_dete_obj, RGB=False)
            x_add, y_add = each_dete_obj.x1, each_dete_obj.y1
            seg_image = model_jyhqx_3.detect(each_im, 'resizedName')
            ...
            if origin_tag == "upring":
                each_dete_obj.do_augment([0, 0, 0, -1.5], jyhqx_1_dete_res.width, jyhqx_1_dete_res.height, is_relative=True)
            elif origin_tag == "downring":
                each_dete_obj.do_augment([0, 0, -1.5, 0], jyhqx_1_dete_res.width, jyhqx_1_dete_res.height, is_relative=True)
