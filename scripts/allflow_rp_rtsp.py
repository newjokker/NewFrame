...
if __name__ == "__main__":
    ...
    for i in range(1, mul_process_num + 1):
        each_cmd_str = r"python3 scripts/all_models_flow.py --scriptIndex {0}-{1} --gpuID {2} --model_list {3} --ignore_history {4} --del_dete_img {5} --del_empty_dir {6}".format(
            mul_process_num, i, gpu_id_list[(i-1)%gpu_num], ','.join(model_list), 'False', 'True', 'False')
